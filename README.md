...
Install a CUDA-enabled PyTorch build from the [official PyTorch instructions](https://pytorch.org/get-started/locally/) when GPU support is required.

### Checkpoints

The following three public checkpoints are available from [Google Drive](https://drive.google.com/drive/folders/1G3Zrn0pFV7XbygEjrYJJYJfgN972cGn8). Download them and put them under the `log/` directory:
...
The converted public AirDC checkpoints use the cleaned module layout:
...
| Dataset | Configs | Checkpoint | Download |
| --- | --- | --- | --- |
| KITTI DC | `train_all_att.yaml`, `val_all_att.yaml`, `test_all_att.yaml` | `log/kt_577_airdc.pth` | [Google Drive](https://drive.google.com/drive/folders/1G3Zrn0pFV7XbygEjrYJJYJfgN972cGn8) |
| MS2 | `train_all_att_ms2.yaml`, `val_all_att_ms2.yaml` | `log/ms2_969_airdc.pth` | [Google Drive](https://drive.google.com/drive/folders/1G3Zrn0pFV7XbygEjrYJJYJfgN972cGn8) |
| Virtual KITTI 2 | `train_all_att_vk2.yaml`, `val_all_att_vk2.yaml` | `log/vk2_2424_airdc.pth` | [Google Drive](https://drive.google.com/drive/folders/1G3Zrn0pFV7XbygEjrYJJYJfgN972cGn8) |

For checkpoints stored elsewhere, either edit `ckpt_path` in the YAML config or override it from the command line.

The released files contain model weights only. They do not include optimizer state, scheduler state, training metrics, dataset paths, or experiment credentials. A checkpoint can be inspected safely with:

```python
import torch

checkpoint = torch.load("log/kt_577_airdc.pth", map_location="cpu", weights_only=True)
...
The test config uses `split: test_completion` and loads `log/kt_577_airdc.pth` by default.
