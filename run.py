...
if config.model_name == "AirDC":
    ...
    if getattr(config, 'load_from_single', None) is not None:
        args.calib_folder = None
        args_val.calib_folder = None
...
if split in ['train','val','test_completion', 'test_prediction', 'test_compare']:
    # Bypass the DataLoader only for single-sample test_completion/test_compare runs.
    if split in ['test_completion', 'test_compare'] and getattr(config, 'load_from_single', None) is not None:
        val_loader = None
        val_visualize_indices = set()
        if rank == 0:
            print(f"\t==> [Warning] `load_from_single`={config.load_from_single}. Bypassing Dataloader for test_completion.")
    else:
...
if __name__ == "__main__":
    if split == 'train':
        # Initialize SwanLab.
        USE_SWAN = not (os.getenv("EXPORT_TO_SWAN", "true").lower() in ("false", "0"))
        if rank == 0:
            if USE_SWAN:
                swanlab.login(api_key='3XZG0KOQlq8GEgxNcUgiG')
                run_swan = swan_lab_init(config)
                print("✅ SwanLab logging is enabled.")
            else:
                print("⚠️ SwanLab logging is disabled.")

        iters_per_epoch = len(train_loader)
        global_step = resume_epoch * iters_per_epoch
        ...
        for ep in range(resume_epoch+1, epochs):
            ...
            # Select fixed visualization indices at the start of each epoch.
            swanlab_train_vis_indices = sorted(list(train_visualize_indices))[:5]
            swanlab_train_images = []
            ...
            for batch_idx, batch_data in enumerate(train_loader):
                ...
                # Collect pseudocolor training images for SwanLab.
                if USE_SWAN and rank == 0 and batch_idx in swanlab_train_vis_indices:
                    from utils.visual_loss import apply_colormap_to_depth
                    import swanlab
                    pred = depth_pred_iters[-1] if isinstance(depth_pred_iters, list) else depth_pred_iters
                    pred_map = pred[0].squeeze().detach()
                    pred_colored = apply_colormap_to_depth(pred_map, stretch_range=(0, config.z_max)).squeeze(0)
                    image = swanlab.Image(pred_colored, caption=f"Train Sample {batch_idx}")
                ...
                with (torch.cuda.amp.autocast(dtype=torch.float32, enabled=True)):
                    ...
                    if rank== 0 and batch_idx > 0 and batch_idx % 20 == 0:
                        ...
                        # Compute the fractional epoch value.
                        fractional_epoch = (ep - 1) + float(batch_idx + 1) / iters_per_epoch
                        epoch_val = round(fractional_epoch, 2)
                        elapsed_min = (time.time() - train_start_time) / 60.0
                        if USE_SWAN:
                            swanlab.log({
                                "train/Time(min)":        elapsed_min,
                                "train/Epoch":            epoch_val,
                                "train/Step":             global_step,
                                "train/TrainLoss":        loss_value.item(),
                                "train/LearningRate":     current_lr,
                                "train/RMSE":             train_metrics_accum["RMSE"] / (batch_idx + 1),
                                "train/MAE":              train_metrics_accum["MAE"] / (batch_idx + 1),
                                "train/iRMSE":            train_metrics_accum["iRMSE"] / (batch_idx + 1),
                                "train/iMAE":             train_metrics_accum["iMAE"] / (batch_idx + 1),
                                "train/near_RMSE":        train_metrics_accum["near_RMSE"] / (batch_idx + 1),
                                "train/near_MAE":         train_metrics_accum["near_MAE"] / (batch_idx + 1),
                                "train/near_iRMSE":       train_metrics_accum["near_iRMSE"] / (batch_idx + 1),
                                "train/near_iMAE":        train_metrics_accum["near_iMAE"] / (batch_idx + 1),
                                "train/far_RMSE":         train_metrics_accum["far_RMSE"] / (batch_idx + 1),
                                "train/far_MAE":          train_metrics_accum["far_MAE"] / (batch_idx + 1),
                                "train/far_iRMSE":        train_metrics_accum["far_iRMSE"] / (batch_idx + 1),
                                "train/far_iMAE":         train_metrics_accum["far_iMAE"] / (batch_idx + 1),
                                "train/abs_rel":    train_metrics_accum["abs_rel"] / (batch_idx + 1),
                                "train/sq_rel":     train_metrics_accum["sq_rel"] / (batch_idx + 1),
                                "train/rms":    train_metrics_accum["rms"] / (batch_idx + 1),
                                "train/log10": train_metrics_accum["log10"] / (batch_idx + 1),
                                "train/delta1": train_metrics_accum["delta1"] / (batch_idx + 1),
                                "train/delta2": train_metrics_accum["delta2"] / (batch_idx + 1),
                                "train/delta3": train_metrics_accum["delta3"] / (batch_idx + 1),
                            }, step=global_step)
                        else:
                            print(
                    ...
                    if rank == 0 and  (batch_idx +1) == len(train_loader) :
                        ...
                        if USE_SWAN and len(swanlab_train_images) > 0:
                            import swanlab
                            swanlab.log({"Train_Depth_Predictions": swanlab_train_images}, step=ep)
        ...
        if rank== 0:
            try:
                ...
                if getattr(config, 'load_from_single', None) is not None:
                    from utils.demo_loader import read_single_demo_folder
                    sample_data = read_single_demo_folder(config.load_from_single, args, device)
                    batch_size = 1
                else:
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(dtype=torch.float16,
            ...
            # Helper function to get batch data either from single folder or dataset
            def get_batch_data(idx):
                if getattr(config, 'load_from_single', None) is not None:
                    from utils.demo_loader import read_single_demo_folder
                    return read_single_demo_folder(config.load_from_single, args, device)
                else:
            ...
            for idx_wr in range(warmup_iterations):
                batch_data = get_batch_data(idx_wr if getattr(config, 'load_from_single', None) is None else 0)
        ...
        if split == "test_compare":
            ...
            for i in range(7):
                ...
                # Run validation to obtain RMSE when load_from_single is disabled.
                rmse_val = "?"
                if getattr(config, 'load_from_single', None) is None:
                    if rank == 0:
                ...
                for val_idx in range(test_iterations):
                    batch_data = get_batch_data(val_idx if getattr(config, 'load_from_single', None) is None else 0)
                    torch.cuda.empty_cache()
                    torch.cuda.reset_peak_memory_stats()
