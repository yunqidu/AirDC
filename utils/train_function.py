...
from .fit_utils import print_on_master,append_to_csv,get_lr
...
def run_val(config,
    ...
    # Use the first five visualization indices as fixed SwanLab samples.
    ...
    with (torch.no_grad()):
        ...
        for index_v, batch_data in enumerate(val_loader):
            ...
            # Collect images for SwanLab logging.
            if getattr(config, 'export_to_swanlab', False) and rank == 0 and index_v in swanlab_vis_indices:
                ...
                # Colorize the first prediction in the batch.
                pred_map = pred[0].squeeze().detach()
                pred_colored = apply_colormap_to_depth(pred_map, stretch_range=(0, args.z_max)).squeeze(0)
        ...
        if getattr(config, 'export_to_swanlab', False) and rank == 0 and len(swanlab_image_list) > 0 and not single_val:
            import swanlab
            swanlab.log({"Val_Depth_Predictions": swanlab_image_list}, step=current_ep)
        ...
        if single_val and rank == 0:
            ...
            avg_val_loss = val_loss_accum / actual_iters
            if USE_SWAN:
                swanlab.log({
                    "val/Time(min)": elapsed_min,
                    "val/Epoch": current_ep,
                    "val/ValLoss": avg_val_loss,
                    "val/LearningRate": get_lr(optimizer),
                    "val/RMSE": val_metrics_avg["RMSE"] ,
                    "val/MAE": val_metrics_avg["MAE"] ,
                    "val/iRMSE": val_metrics_avg["iRMSE"] ,
                    "val/iMAE": val_metrics_avg["iMAE"] ,
                    "val/near_RMSE": val_metrics_avg["near_RMSE"] ,
                    "val/near_MAE": val_metrics_avg["near_MAE"] ,
                    "val/near_iRMSE": val_metrics_avg["near_iRMSE"] ,
                    "val/near_iMAE": val_metrics_avg["near_iMAE"] ,
                    "val/far_RMSE": val_metrics_avg["far_RMSE"] ,
                    "val/far_MAE": val_metrics_avg["far_MAE"] ,
                    "val/far_iRMSE": val_metrics_avg["far_iRMSE"] ,
                    "val/far_iMAE": val_metrics_avg["far_iMAE"] ,
                    "val/abs_rel": val_metrics_avg["abs_rel"],
                    "val/sq_rel": val_metrics_avg["sq_rel"],
                    "val/rms": val_metrics_avg["rms"],
                    "val/log10": val_metrics_avg["log10"],
                    "val/delta1": val_metrics_avg["delta1"],
                    "val/delta2": val_metrics_avg["delta2"],
                    "val/delta3": val_metrics_avg["delta3"],
                }, step=current_ep)
        actual_iters = test_iterations if (test_iterations is not None and test_iterations < len(val_loader)) else len(val_loader)
