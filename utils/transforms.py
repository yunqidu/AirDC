...
class NormalizeNumpyArray(object):
    ...
    def __call__(self, img):
        ...
        if not (_is_numpy_image(img)):
            raise TypeError('img should be ndarray. Got {}'.format(type(img)))
        # Improve efficiency if this transform becomes a bottleneck.
        print(img.shape)
        for i in range(3):
...
class BottomCrop(object):
    """Bottom-crop a numpy.ndarray and return the cropped image with crop metadata.

    Args:
        size (sequence or int): Target crop size. If size is an int, use a square crop (size, size).
    """
    def __init__(self, size):
        if isinstance(size, numbers.Number):
            self.size = (int(size), int(size))
    ...
    @staticmethod
    def get_params(img, output_size):
        """Compute bottom-crop parameters.

        Args:
            img (numpy.ndarray (H x W x C) or (H x W)): Image to crop.
            output_size (tuple): Target crop size (th, tw).

        Returns:
            tuple: Crop parameters (crop_top, crop_left, th, tw).
        """
        h = img.shape[0]
        w = img.shape[1]
        th, tw = output_size
    ...
    def __call__(self, img):
        ...
        Returns:
            ...
            crop_info (dict): Crop metadata dictionary containing 'crop_top', 'crop_left', 'crop_height', 'crop_width'
        """
        if not (_is_numpy_image(img)):
            raise TypeError('img must be a numpy.ndarray, got {}'.format(type(img)))

        crop_top, crop_left, crop_height, crop_width = self.get_params(img, self.size)

        if img.ndim == 3:
            cropped = img[crop_top:crop_top + crop_height, crop_left:crop_left + crop_width, :]
        elif img.ndim == 2:
            cropped = img[crop_top:crop_top + crop_height, crop_left:crop_left + crop_width]
        else:
            raise RuntimeError('img must be a 2D or 3D array, got ndim= {}'.format(img.ndim))
...
class RandomCrop(object):
    ...
    def get_params(img, output_size):
        ...
        th, tw = output_size
        if th > h or tw > w:
            raise ValueError("Requested crop size {} is larger than the input size {}".format(output_size, (h, w)))

        # Generate a random crop location.
        crop_top = np.random.randint(0, h - th + 1)
        crop_left = np.random.randint(0, w - tw + 1)
        return crop_top, crop_left, th, tw
    ...
    def __call__(self, img):
        """
        Args:
            img (numpy.ndarray): Input image with shape (H, W, C) or (H, W).

        Returns:
            tuple: (cropped_img, crop_info)
                cropped_img is the cropped image
                crop_info stores crop position and size
        """
        if not (_is_numpy_image(img)):
            raise TypeError('img should be ndarray. Got {}'.format(type(img)))
            # Get crop parameters.
        crop_top, crop_left, crop_h, crop_w = self.get_params(img, self.size)

        # Apply the crop.
        if img.ndim == 3:
            cropped = img[crop_top:crop_top + crop_h,
        ...
        # Build the crop metadata dictionary.
        crop_info = {
            'crop_top': crop_top,
            'crop_left': crop_left,
            'crop_height': crop_h,
        ...
        return cropped, crop_info  # Return the tuple.
